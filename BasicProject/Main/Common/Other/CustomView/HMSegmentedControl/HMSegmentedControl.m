...
-(void)setBootomLine{
    UIView *BootomLine = [[UIView alloc]initWithFrame:FRAME(0, self.bottom, SCREEN_WIDTH,0.5)];
    BootomLine.backgroundColor = COL_GRAY_LINE;
    [self addSubview:BootomLine];
}

-(void)setVerticalCenterLine{
    UIView *lineView = [[UIView alloc]initWithFrame:FRAME(self.width/2.0, 0, 1,self.height-1)];
    lineView.backgroundColor = COL_GRAY_LINE;
...
- (void)setDefaults {
    self.font = [UIFont fontWithName:@"STHeitiSC-Light" size:PX(36)];
...
- (CGRect)frameForSelectionIndicator {// 线位置
    ...
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        ...
        return CGRectMake(x, self.frame.size.height-2, stringWidth, self.selectionIndicatorHeight);
    } else {
        return CGRectMake(self.segmentWidth * self.selectedIndex, self.frame.size.height-2, self.segmentWidth, self.selectionIndicatorHeight);
    }
}
