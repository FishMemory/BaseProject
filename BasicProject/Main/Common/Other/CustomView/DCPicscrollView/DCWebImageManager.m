...
- (NSString *)cachePath {
    if (!_cachePath) {
        ...
        NSString *firtstBannerCachePath = [sandBoxPath stringByAppendingPathComponent:firtstBannerCache];
        NSFileManager *fileManager = [NSFileManager defaultManager];
        BOOL isExist =  [fileManager fileExistsAtPath:firtstBannerCachePath];
        if (!isExist) {
            [fileManager createDirectoryAtPath:firtstBannerCachePath withIntermediateDirectories:YES attributes:nil error:nil];
        }
        _cachePath = firtstBannerCachePath;
    }
//    if (!_cachePath) {
