...
// 保存文件名
#define HSFileName(url) [[url md5String] stringByAppendingString:@".mp4"]
...
{
//    NSLog(@"获取已下载长度%d",HSDownloadLength(url));
...
{
    NSDictionary *lengthPathDic =  [NSDictionary dictionaryWithContentsOfFile:HSTotalLengthFullpath];
    NSString * key = HSFileName(url);
    NSNumber *length =  [lengthPathDic objectForKeyCheck:key];
    if (length) {
