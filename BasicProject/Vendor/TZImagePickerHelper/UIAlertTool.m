...
+(void)showActionSheetWithTitle:(NSString *)title message:(NSString *)message cancelTitle:(NSString *)cancelTitle oneTitle:(NSString *)oneTitle twoTitle:(NSString *)twoTitle  one:(void (^)())one   two:(void (^)())two cancle:(void (^)())cancel
{
    ...
    if (oneTitle) {
        UIAlertAction *otherAction = [UIAlertAction actionWithTitle:oneTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            if (one) {
                one();
            }
        }];
    ...
    if (twoTitle) {
        UIAlertAction *otherAction = [UIAlertAction actionWithTitle:twoTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            if (two) {
                two();
            }
        }];
