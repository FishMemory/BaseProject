...
-(UIImage *)drawImage{
    UIGraphicsBeginImageContext(CGSizeMake(self.size.width+40, self.size.height));
    //cg的坐标是左下角 将坐标调整为UIKit一样
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextTranslateCTM(context, 0.0, self.size.height);//向上平移
    CGContextScaleCTM(context, 1.0, -1.0);////在y轴缩放-1相当于沿着x张旋转180
    
    CGContextDrawImage(context, CGRectMake(20.0f, 0.0f, self.size.width, self.size.height), self.CGImage);
