...
//    }
    ...
    for (int i = 0; i<3; i ++) {
        ...
        if (i == 0 && self.dataArry!=nil && self.dataArry.count > 1) {
            [imageView sd_setImageWithURL:[NSURL  URLWithString:self.dataArry[self.dataArry.count - 1]] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
//            imageView.backgroundColor = self.dataArry[self.dataArry.count - 1];//左边
        }
        if (i == 1 && self.dataArry!=nil && self.dataArry.count > 0) {
            [imageView sd_setImageWithURL:[NSURL  URLWithString: self.dataArry[0]] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
...
//            imageView.backgroundColor = self.dataArry[1];//右边
            [imageView sd_setImageWithURL:[NSURL  URLWithString:self.dataArry[1]] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
        }
...
//    [self addSubview:self.pageControl];
    ...
    _pageLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.width-PX(80), self.height-PX(80), PX(70), PX(70))];
    _pageLabel.textColor = [UIColor whiteColor];
    _pageLabel.font = FONT(PX(24));
    _pageLabel.backgroundColor = rgba(0, 0, 0, 0.8);
    _pageLabel.textAlignment = NSTextAlignmentCenter;
    [_pageLabel setViewWithRadius:PX(35)];
    _pageLabel.text = [NSString stringWithFormat:@"%@/%@",@(1),@(_imageCount)];
...
//    centerImageView.backgroundColor =self.dataArry[_currentImageIndex];
    NSString *centerImageUrl = [self.dataArry objectAtIndexCheck:_currentImageIndex];
    [centerImageView sd_setImageWithURL:[NSURL  URLWithString:centerImageUrl] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
...
//    leftImageView.backgroundColor  = self.dataArry[leftImageIndex] ;
    NSString *leftImageUrl = [self.dataArry objectAtIndexCheck:leftImageIndex];
    [leftImageView sd_setImageWithURL:[NSURL  URLWithString:leftImageUrl] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
    
//    rightImageView.backgroundColor = self.dataArry[rightImageIndex] ;
    NSString *rightImageUrl = [self.dataArry objectAtIndexCheck:rightImageIndex];
    [rightImageView sd_setImageWithURL:[NSURL  URLWithString:rightImageUrl] placeholderImage:ImageNamed(DEFAULT_PLACE_SCROLL)];
    
    [self.scrollView setContentOffset:CGPointMake(self.scrollView.frame.size.width, 0) animated:NO];
    self.pageControl.currentPage = self.currentImageIndex;
    _pageLabel.text = [NSString stringWithFormat:@"%@/%@",@(_currentImageIndex+1),@(_imageCount)];
}
