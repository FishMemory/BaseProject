...
-(void)creatUI:(NSMutableArray *)tagArr{
    ...
    [self.tagArr enumerateObjectsUsingBlock:^(id value, NSUInteger idx, BOOL *stop) {
        ...
        if (self->previousFrame.origin.x + self->previousFrame.size.width + Size_str.width + K_Tag_Right_Margin > JF_Screen_Width) {
            
            newRect.origin = CGPointMake(10, self->previousFrame.origin.y + Size_str.height + K_Tag_Bottom_Margin);
            self->tagView_height += Size_str.height + K_Tag_Bottom_Margin;
        }
        else {
            newRect.origin = CGPointMake(self->previousFrame.origin.x + self->previousFrame.size.width + K_Tag_Right_Margin, self->previousFrame.origin.y);
            
        }
        newRect.size = Size_str;
        [tagLabel setFrame:newRect];
        self->previousFrame=tagLabel.frame;
        
        //改变控件高度
        if (idx==self.tagArr.count-1) {
            [self setHight:self andHight:self->tagView_height+Size_str.height + K_Tag_Bottom_Margin];
        }
